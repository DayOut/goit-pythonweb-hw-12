...
def test_signup_same_email(client, monkeypatch):
    ...
    assert (
        response.json()["detail"] == "User with this email already exists"
    )
...
async def test_confirm_email_already_confirmed(client, monkeypatch):
    ...
    assert response.status_code == 200
    assert response.json()["message"] == "Your email address is already confirmed"
...
async def test_request_email(client, monkeypatch):
    ...
    assert response.status_code == 200
    assert response.json()["message"] == "Check your email for confirmation instructions"
